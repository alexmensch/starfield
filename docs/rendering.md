...
linearly by magnitude. See §Chart mode below for the full feature.
...
contour** along the magnitude limit (see §Chart mode). The diffuse glow
doesn't suit a paper-chart aesthetic, but a thin ink line tracking
"where the integrated MW would equal the visible magnitude limit"
reads as an exact paper-atlas equivalent of the volumetric band.
