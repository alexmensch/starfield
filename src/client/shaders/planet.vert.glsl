...
// (0=gas giant soft edge, 1=rocky hard edge).
in vec3 iPosition;
in float iRadiusPc;
in vec3 iColour;
in float iSolidity;
...
out vec3 vColour;
out float vSolidity;
// vQuadUv runs from 0 at body centre to 1 at the body edge (the quad
// corner). length(vQuadUv) gives the fragment's radial distance in
// body-radius units, which the fragment shader's body falloff
// consumes directly. Atmospheric haloes / banding / textures all
// depend on a future close-zoom affordance and are deferred.
out vec2 vQuadUv;
...
void main() {
  ...
  // by projecting outside the clip volume.
  if (d <= 0.0) {
  ...
  vec3 cornerView = viewCenter.xyz + vec3(aCorner * vsHalf * 2.0, 0.0);
  gl_Position = projectionMatrix * vec4(cornerView, 1.0);

  vColour = iColour;
  vSolidity = iSolidity;
  // aCorner is [-0.5, 0.5] across the quad. Map to [-1, 1] in
  // body-radius units so length(vQuadUv) ranges [0, √2] across the
  // quad and equals 1 exactly at the body edge along ±x or ±y.
  vQuadUv = aCorner * 2.0;
