...
//
// The pick path delegates to `Stellata.pickStarHit`, which already wraps
// the canonical `pickStar` reducer and lifts its winning idx into a
// `HoverHit` (tier + camera distance). The formatter is the pure
// `formatStarHover` against a bound context bundle — no behavioural
// change from the prior inline form.
