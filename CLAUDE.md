...
scripts/
  ...
  build-binaries.py       WDS + ORB6 → data/multiples.tsv cross-match (stdlib Python; runs first inside build:catalog)
...
data/                                All large catalogs tracked via Git LFS.
  ...
  wds_summ.txt                       Washington Double Star summary (~22 MB, LFS)
  wds_notes.txt                      WDS notes / HIP prose cross-refs (~5 MB, LFS)
  wds_refs.txt                       WDS discoverer codes + references (~3 MB, LFS)
  orb6_orbits.txt                    Sixth Catalog of Orbits of Visual Binary Stars (~1 MB, LFS)
  multiples-overrides.tsv            hand-curated multiple-star edge cases (small; not LFS)
  molecular-clouds/
...
# additional generated outputs under data/
data/multiples.tsv        generated by build-binaries.py (gitignored)
data/wds_upload.csv       Gaia ADQL upload, regenerated each build (gitignored)
src/
  ...
  client/
    ...
    kepler-solver.ts      shared Newton solver for M = E − e·sin(E); used by ephemeris.ts + binary-orbit-pure.ts
    binary-orbit-pure.ts  Thiele-Innes sky-plane projection + ΔR(t)−R(J2000) baseline for catalog binaries (dch.9)
    astronomy-constants.ts  canonical AU_PC / AU_PER_PC / AU_KM / KM_PC / R_SUN_PC / J2000_JD
...
```bash
npm run build:binaries  # cross-match WDS/ORB6 → data/multiples.tsv (idempotent; runs first inside build:catalog)
